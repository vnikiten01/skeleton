...
# USE FOR POSTGRES AND MAKE SURE TO UNCOMMENT LINES IN DATABASE
...
namespace :sdb do
  desc "Create the database at #{SDB_NAME}"
  task :create do
    puts "Creating database #{SDB_NAME} if it doesn't exist..."
    # exec("createdb #{DB_NAME}")
    exec "echo #{SDB_NAME} "
  end

  desc "Drop the database at #{SDB_NAME}"
  task :drop do
    puts "Dropping database #{SDB_NAME}..."
    exec("dropdb #{SDB_NAME}")
  end
